#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Converted from DRLMAPF_A3C_RNN_episode_logging_fixed_v2.ipynb.

Run from a terminal to capture full stdout/stderr and native crash dumps.
Recommended:
  python -u DRLMAPF_A3C_RNN_episode_logging_debug.py 2>&1 | tee train_debug.log
"""
...
class Worker:
    ...
    def shouldRun(self, coord, episode_count):
        if TRAINING:
            return (not coord.should_stop()) and (episode_count < MAX_EPISODES)
        else:
    ...
    def work(self,max_episode_length,gamma,sess,coord,saver):
        global episode_count, last_saved_ckpt, swarm_reward, episode_rewards, episode_lengths, episode_mean_values, episode_invalid_ops,episode_wrong_blocking #, episode_invalid_goals
        total_steps, i_buf = 0, 0

        def maybe_save_checkpoint(current_episode):
            current_episode_int = int(np.floor(current_episode))
            target_ckpt = (current_episode_int // CHECKPOINT_EVERY) * CHECKPOINT_EVERY
            if target_ckpt <= 0:
                return
            with save_lock:
                global last_saved_ckpt
                if target_ckpt > last_saved_ckpt:
                    print('Saving Model', end='\n')
                    saver.save(sess, model_path+'/model-'+str(target_ckpt)+'.cptk')
                    last_saved_ckpt = target_ckpt
        ...
        with sess.as_default(), sess.graph.as_default():
            while self.shouldRun(coord, episode_count):
                ...
                if episode_count<PRIMING_LENGTH or demon_probs[self.metaAgentID]<DEMONSTRATION_PROB:
                    ...
                    if self.workerID==1:
                        maybe_save_checkpoint(episode_count)
                    global rollouts
                    ...
                    if(self.agentID==1):
                        ...
                        try:
                            mstar_path=expert_find_path(world,start_positions,goals,2,5)
                    ...
                    if rollouts[self.metaAgentID] is not None:
                        ...
                        if self.agentID==1:
                            ...
                            if PRINT_EVERY_EPISODE and ((int(np.floor(episode_count)) + 1) % EPISODE_LOG_INTERVAL == 0):
                                current_learning_rate = sess.run(lr, feed_dict={global_step: episode_count})
                                episode_idx = int(np.floor(episode_count)) + 1
                                print(
                                    "[Episode {:d}] mode=IL | lr={:.6e} | reward={:.2f} | loss={:.6f}".format(
                                        episode_idx,
                ...
                if not TRAINING:
                    ...
                    if self.workerID == 1 and PRINT_EVERY_EPISODE and ((int(np.floor(episode_count)) + 1) % EPISODE_LOG_INTERVAL == 0):
                        episode_idx = int(np.floor(episode_count)) + 1
                        current_learning_rate = sess.run(lr, feed_dict={global_step: episode_count})
                        total_loss = float(v_l + p_l + valid_l + b_l)
                        print(
                            "[Episode {:d}] mode=RL | lr={:.6e} | reward={:.2f} | loss={:.6f}".format(
                                episode_idx,
                                current_learning_rate,
                                episode_reward,
                                total_loss
...
RUN_NAME   = "2026-04-18_obsfull_rlil_s1_run04"
...
with tf.device(DEVICE):
    ...
    with tf.Session(config=config) as sess:
        ...
        if load_model == True:
            ...
            p=p[:p.find('.')]
            episode_count=int(p)
            last_saved_ckpt = (episode_count // CHECKPOINT_EVERY) * CHECKPOINT_EVERY
        ...
        for ma in range(NUM_META_AGENTS):
            for worker in workers[ma]:
                ...
                def worker_work(w=worker):
                    try:
                        return w.work(max_episode_length, gamma, sess, coord, saver)
                    except Exception as e:
                        print("\n[Worker {} crashed] {}".format(w.workerID, e), file=sys.stderr, flush=True)
                        traceback.print_exc()
                        try:
                ...
                print("Starting worker " + str(worker.workerID), flush=True)
                t = threading.Thread(target=worker_work, name="worker-thread-{}".format(worker.workerID))
                t.start()
